...
> [!IMPORTANT]
> このリポジトリは、オリジナルの [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) を基にした**非公式の派生フォーク**です。オリジナルプロジェクト、コア実装、上流の開発成果は、元リポジトリのメンテナーとコントリビューターに帰属します。このフォーク固有の Grok Free 変更および GHCR イメージは上流プロジェクトの公式リリースではなく、上流からのサポートも提供されません。
...
オリジナルの [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) とこの派生版は MIT ライセンスで配布されています。上流の著作権表示と許諾表示は [LICENSE](LICENSE) にそのまま保持されています。
